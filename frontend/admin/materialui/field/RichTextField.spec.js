...
describe('stripTags', () => {
  ...
  it('should strip HTML tags even with attributes', () => {
    ...
    assert.equal(
      removeTags('<a target="_blank" href="http://www.zombo.com">Zombo</a>'),
      'Zombo'
    );
  });
...
describe('<RichTextField />', () => {
  it('should render as HTML', () => {
    ...
    assert.equal(
      html(wrapper),
      '<span class="MuiTypography-root MuiTypography-body2"><span><h1>Hello world!</h1></span></span>'
    );
  });
  ...
  it('should strip HTML tags if stripTags is set to true', () => {
    ...
    assert.equal(
      html(wrapper),
      '<span class="MuiTypography-root MuiTypography-body2">Hello world!</span>'
    );
  });
  ...
  it('should use custom className', () =>
    assert.deepEqual(
      shallow(
        <RichTextField record={{ foo: true }} source="body" className="foo" />
      ).prop('className'),
      'foo'
    ));
});
