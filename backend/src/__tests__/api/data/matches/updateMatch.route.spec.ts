...
describe('PUT Match Route', () => {
  ...
  describe.only('PUT api/matches', () => {
    it('When asked for teams, Then should retrieve and receive 200 response', async () => {
      // READ
      const url = '';
      // /competitions/cSlug/seasons/sSlug/matches
      const { data: rData, status: rStatus } = await testSetup
        .getHttpClient()
        .get(url);

      // UPDATE
      const { data: uData, status: uStatus } = await testSetup
        .getHttpClient()
        .put('/matches', {
          data: {
            ...rData,
